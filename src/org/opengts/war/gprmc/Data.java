...
public class Data extends CommonServlet {
	...
	/* default account id */
	// The URL variable "?dev=<id>" is used to allow this module the ability to
	// uniquely
	// identify a reporting phone. The reported "<id>" provided by the phone is
	// used
	// as the "DeviceID" for the following AccountID. This value can be
	// overridden in
	// the Servlet 'webapp.conf' file.
	public static String DefaultAccountID = "";

	/* minimum required speed */
	// GPS receivers have a tendency to appear 'moving', even when sitting
	// stationary
	// on your desk. This filter is a 'speed' threshold which is used to force
	// the
	// reported speed to '0' if it falls below this value. This value can be
	// overridden
	...
	/* Compass headings */
	private static String COMPASS_HEADING[] = new String[] { "N", "NE", "E", "SE", "S", "SW", "W", "NW" };
	...
	private static String PARM_STATUS[] = { "code", "sc", "pointType" }; // event/status
																			// code
	private static String PARM_ALTITUDE[] = { "alt", "altitude" }; // altitude
																	// (meters)
	private static String PARM_ODOMETER_KM[] = { "odom", "odometer", "odo" }; // odometer
																				// (kilometers)
	private static String PARM_ODOMETER_MI[] = { "mile", "miles", "odomi" }; // odometer
																				// (miles)
	private static String PARM_DISTANCE_KM[] = { "dist", "distance", "trip" }; // distance
																				// (kilometers)
	private static String PARM_DISTANCE_MI[] = { "tripm", "distmiles", "tripmi" }; // distance
																					// (kilometers)
	private static String PARM_ADDRESS[] = { "addr", "address" }; // reverse-geocoded
																	// address
	private static String PARM_DRIVER[] = { "drv", "driver" }; // driver
	private static String PARM_MESSAGE[] = { "msg", "message" }; // message
	private static String PARM_NUM_SATS[] = { "sat", "sats", "numsats" }; // number
																			// of
																			// satellite
	private static String PARM_HDOP[] = { "hdop" }; // horizontal dilution of
													// precision
	private static String PARM_HORZ_ACC[] = { "hacc", "accuracy", "acc" }; // horizontal
																			// accuracy
																			// (meters)
	private static String PARM_VERT_ACC[] = { "vacc" }; // vertical accuracy
														// (meters)
	private static String PARM_BATT_LEVEL[] = { "batl", "battl", "battlevel", "batt" }; // battery
																						// level
																						// (%)
	private static String PARM_BATT_VOLTS[] = { "batv", "battv", "battvolts" }; // battery
																				// voltage
	private static String PARM_BATT_TEMPC[] = { "batc", "battc", "batttemp" }; // Battery
																				// tempC
	...
	private static String PARM_CELLID_RXLEV[] = { "rxlev", "rxlvl", "rxl" }; // CellID:
																				// rxlev
	private static String PARM_CELLID_ARFCN[] = { "arfcn", "arf" }; // CellID:
																	// arfcn

	// -- $GPRMC field key
	private static String PARM_GPRMC[] = { "gprmc", "rmc", "cds", "nmea" }; // $GPRMC
																			// data

	// -- these are ignored if PARM_GPRMC is defined
	private static String PARM_DATE[] = { "date", "datetime", "dt" }; // date
																		// (YYYYMMDD)
	...
	private static String PARM_SPEED_MPH[] = { "mph", "speedmph" }; // speed
																	// (mph)
	private static String PARM_HEADING[] = { "head", "heading", "direction", "dir" }; // heading
																						// (degrees)
	...
	public static final String CONFIG_DATE_FORMAT = DEVICE_CODE + ".dateFormat"; // "YMD",
																					// "DMY",
																					// "MDY"
	...
	public static final String CONFIG_PARM_DATE = DEVICE_CODE + ".parm.date"; // epoch,
																				// YYYYMMDD,
																				// DDMMYYYY,
																				// MMDDYYYY
	...
	public static final String CONFIG_PARM_NUM_SATS = DEVICE_CODE + ".parm.sats"; // number
																					// satellites
	public static final String CONFIG_PARM_HDOP = DEVICE_CODE + ".parm.hdop"; // HDOP
	public static final String CONFIG_PARM_HORZ_ACC = DEVICE_CODE + ".parm.hacc"; // horizontal
																					// accuracy
	public static final String CONFIG_PARM_VERT_ACC = DEVICE_CODE + ".parm.vacc"; // vertical
																					// accuracy
	public static final String CONFIG_PARM_BATT_LEVEL = DEVICE_CODE + ".parm.battlevel"; // battery
																							// level
	public static final String CONFIG_PARM_BATT_VOLTS = DEVICE_CODE + ".parm.battvolts"; // battery
																							// voltage
	public static final String CONFIG_PARM_BATT_TEMPC = DEVICE_CODE + ".parm.batttemp"; // battery
																						// temp-C
	public static final String CONFIG_PARM_TEMPERATURE = DEVICE_CODE + ".parm.temperature"; // temperature
	public static final String CONFIG_PARM_CELLID_MCC = DEVICE_CODE + ".parm.mcc"; // CellID:
																					// mcc
	public static final String CONFIG_PARM_CELLID_MNC = DEVICE_CODE + ".parm.mnc"; // CellID:
																					// mnc
	public static final String CONFIG_PARM_CELLID_LAC = DEVICE_CODE + ".parm.lac"; // CellID:
																					// lac
	public static final String CONFIG_PARM_CELLID_CID = DEVICE_CODE + ".parm.cid"; // CellID:
																					// cid
	public static final String CONFIG_PARM_CELLID_TAV = DEVICE_CODE + ".parm.tav"; // CellID:
																					// tav
	public static final String CONFIG_PARM_CELLID_RAT = DEVICE_CODE + ".parm.rat"; // CellID:
																					// rat
	public static final String CONFIG_PARM_CELLID_RXLEV = DEVICE_CODE + ".parm.rxlev"; // CellID:
																						// rxlev
	public static final String CONFIG_PARM_CELLID_ARFCN = DEVICE_CODE + ".parm.arfcn"; // CellID:
																						// arfcn
	public static final String CONFIG_PARM_EMAIL = DEVICE_CODE + ".parm.email"; // email
																				// address
	// ------------------------------------------------------------------------
	// ------------------------------------------------------------------------
	...
	static {
		...
		for (int i = 0; i < UniqueIDPrefix.length; i++) {
			...
			} else if (!UniqueIDPrefix[i].endsWith("_") && !UniqueIDPrefix[i].endsWith("-")) {
				UniqueIDPrefix[i] += "_";
			}
		}
		...
		} else if (dateFmt.equalsIgnoreCase("EPOCH")) {
		...
		} else if (dateFmt.equalsIgnoreCase("YMDhms")) {
	...
	private static int TranslateStatusCode(String statusCodeStr) {
		...
		// TODO: Status code translations are device dependent, thus this
		// section will
	 ...
	 **/
	private Device loadDevice(String ipAddr, String mobileID, String accountID, String deviceID, String authCode) {
		Device device = null;

		/* DeviceID used as MobileID? */
		if (StringTools.isBlank(mobileID) && StringTools.isBlank(accountID) && !StringTools.isBlank(deviceID)) {
			mobileID = deviceID;
			deviceID = null;
		}
		...
		if (!StringTools.isBlank(mobileID)) {
			...
			for (int i = 0; (i < UniqueIDPrefix.length) && (device == null); i++) {
				String uid = UniqueIDPrefix[i] + mobileID; // ie:
															// "gprmc_123456789012345"
				try {
			...
			if (device == null) {
				...
				// Print.sysPrintln("NotFound: now=%d id=%s",
				// DateTime.getCurrentTimeSec(), uid);
			...
			if (StringTools.isBlank(accountID)) {
				...
				if (StringTools.isBlank(accountID)) {
					...
					Data.logError(null, "(has '" + CONFIG_PARM_ACCOUNT + "' or '" + CONFIG_PARM_MOBILE
							+ "' been properly configured in 'webapp.conf'?)");
					return null;
				}
			}
		...
		// -- This may be used to prevent rogue hackers from spoofing data
	...
	private void _doWork(final boolean isPost, HttpServletRequest request, HttpServletResponse response)
		...
		String password = AttributeTools.getRequestString(request, PARM_PASSWORD, ""); // not
																						// currently
																						// used
		...
		if (StringTools.isBlank(_gprmcStr) && StringTools.isBlank(_statusStr) && StringTools.isBlank(_latitude)
		...
		// -- this specifically tries to identify the SourceForge "GpsTracker"
		// format,
		// - which uses distance units in Feet/Miles rather than
		// Meters/Kilometers.
		// -
		...
		} else if (isUSUnits && (odometerKM > 0.0)) {
		...
		} else if (isUSUnits && (distanceKM > 0.0)) {
		...
		} else if (isUSUnits && (speedKPH > 0.0)) {
		...
		double battTempC = StringTools.parseDouble(_battTempC, BAD_TEMP); // degrees
																			// C
		if ((_battLevel != null) && _battLevel.indexOf(".") < 0) { // fix NPE
																	// 2.5.6-B18
			// -- "&batt=50": no decimal-point in battery level, assume integer
			// range 0..100
			battLevel /= 100.0;
		} else if (battLevel > 1.0) {
			// -- "&batt=50.3": found decimal-point, but greater than "1.0",
			// assume decimal range 1.0..100.0
			// - this may incorrectly re-adjust levels on devices that show a
			// battery-level grater than 100%
		...
		if (!StringTools.isBlank(gprmcStr)) {
			if (gprmcStr.startsWith("GPRMC") || gprmcStr.startsWith("GPGGA")) {
				// allow "GPRMC,....", adjust to "$GPRMC,...."
				gprmcStr = "$" + gprmcStr;
			} else if (gprmcStr.startsWith("$GPRMC") || gprmcStr.startsWith("$GPGGA")) {
				// ok as-is
			} else {
			...
			boolean ignoreChecksum = (gprmcStr.indexOf("*") >= 0) ? // ignore
																	// checksum
																	// if not
																	// present
			...
			if ((latitude == INVALID_LATLON) || (longitude == INVALID_LATLON)) {
				...
				Data.logError(null, "(is '" + CONFIG_PARM_LATITUDE + "'/'" + CONFIG_PARM_LONGITUDE
						+ "' properly configured in 'webapp.conf'?)");
				isValidGPS = false;
				latitude = 0.0;
				longitude = 0.0;
		...
		if ((speedKPH <= 0.0) || (headingDeg < 0.0)) {
			// We're either not moving, or the GPS receiver doesn't know the
			// heading
		...
		if (odometerKM <= 0.0) {
			// calculate odometer
			odometerKM = (ESTIMATE_ODOMETER && isValidGPS) ? device.getNextOdometerKM(new GeoPoint(latitude, longitude))
		...
		Data.logInfo(
				"Status   : [" + StatusCodes.GetHex(statusCode) + "] " + StatusCodes.GetDescription(statusCode, null));
		...
		if (!isValidGPS && (statusCode == StatusCodes.STATUS_LOCATION)) {
			// -- ignore invalid GPS fixes that have a simple 'STATUS_LOCATION'
			// status code
			...
			if (zone != null) {
				for (Device.GeozoneTransition z : zone) {
					...
					if (EventData.isValidTemperature(battTempC)) {
						...
						zoneEv.setBatteryTemp(battTempC); // Comment this line
															// if a compiler
															// error occurs
					}
					if (EventData.isValidTemperature(tempC)) {
		...
		if ((statusCode != StatusCodes.STATUS_LOCATION) || (eventCount == 0)) {
			...
			if (EventData.isValidTemperature(battTempC)) {
				// -- BatteryTemp defined only in OpenGTS_2.5.5+
				evdb.setBatteryTemp(battTempC); // Comment this line if a
	...
	private long _parseFixtime(String dateStr, String timeStr) {
		...
		if (DateFormat == DATE_FORMAT_EPOCH) {
			...
			if (StringTools.isBlank(epochStr)) {
				...
				if (timestamp > MaxTimestamp) { // (long)Integer.MAX_VALUE * 2)
					timestamp /= 1000L; // timestamp was specified in
										// milliseconds
				}
		...
		if (((DateFormat == DATE_FORMAT_YMDhms) || (DateFormat == DATE_FORMAT_YMD))
			...
			if (StringTools.isBlank(timeStr)) {
				...
				// &date=20121104123432 <-- YYYYMMDDhhmmss
				// &date=2012/11/04,12:34:32 <-- YYYY/MM/DD,hh:mm:ss
				// &date=2012-11-04T12:34:32+00:00 <-- YYYY-MM-DDThh:mm:ssZ (GPX
				// format)
				// &date=2012-11-24 12:34:32+00:00 <-- YYYY-MM-DD hh:mm:ssZ
				// (TODO:)
				dtStr = dateStr;
			} else {
				...
				// &time=2012-11-04T12:34:32+00:00 <-- YYYY-MM-DDThh:mm:ssZ (GPX
				// format)
				// &time=2012-11-24 12:34:32+00:00 <-- YYYY-MM-DD hh:mm:ssZ
				// (TODO:)
				dtStr = timeStr;
			}
			try {
				DateTime dt = DateTime.parseArgumentDate(dtStr, gmtTimeZone, DateTime.DefaultParsedTime.CurrentTime);
		...
		if (DateFormat == DATE_FORMAT_YMD) {
			if (dateLen == 8) {
				...
				DD = StringTools.parseInt(dateStr.substring(4, 5), 0); // fixed
																		// 2011/03/14
			}
	...
	private double _parseLatitude(String latStr) {
		...
		if (StringTools.isBlank(latStr)) {
		...
		} else if ((latStr.length() >= 6) && Character.isDigit(latStr.charAt(0)) && (latStr.charAt(4) == '.')) {
			...
			if (_lat < 9000.0) {
				...
				char d = Character.toUpperCase(latStr.charAt(latStr.length() - 1)); // last
																					// character
																					// N/S
				return (d == 'S') ? -lat : lat;
			} else {
	...
	private double _parseLongitude(String lonStr) {
		...
		if (StringTools.isBlank(lonStr)) {
			return INVALID_LATLON;
		} else if ((lonStr.length() >= 7) && Character.isDigit(lonStr.charAt(0)) && (lonStr.charAt(5) == '.')) {
			...
			if (_lon < 18000.0) {
				...
				char d = Character.toUpperCase(lonStr.charAt(lonStr.length() - 1)); // last
																					// character
																					// E/W
				return (d == 'W') ? -lon : lon;
			} else {
	...
	private double _parseHeading(String headingStr) {
		if (StringTools.isBlank(headingStr)) {
			return 0.0;
		} else if (Character.isLetter(headingStr.charAt(0))) {
	...
	private void plainTextResponse(HttpServletResponse response, String errMsg) throws ServletException, IOException {
